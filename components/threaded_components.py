...
class ThreadedComponent(BaseComponent, ABC):
    ...
    def __init__(self, thread_spec: str, threaded_length: Quantity, material: Material) -> None:
        ...
        Args:
            thread_spec: Thread specification (e.g., "1/4-20 UNC")
...
class Fastener(ThreadedComponent):
    ...
    def __init__(self, thread_spec: str, length: Quantity, threaded_length:
        Quantity, head_diameter: Quantity, head_height: Quantity, is_flat:
        ...
        super().__init__(thread_spec=thread_spec, threaded_length=
            threaded_length, material=material)
        self._length = length
        self._head_diameter = head_diameter
        self._head_height = head_height
        self._is_flat = is_flat
...
class Nut(ThreadedComponent):
    ...
    def validate_geometry(self) ->bool:
        ...
        Raises:
            ValueError: If geometry is invalid
        """
        if (self._width_across_flats.magnitude <= 0 or self._height.
            magnitude <= 0):
            raise ValueError('All dimensions must be positive')
        if self._width_across_flats <= self.pitch_diameter:
            raise ValueError(
                'Width across flats must be greater than pitch diameter')
        return True
